...
| **`destinationAccount`** (_Optional_) | `string`                                                                                                                                    | Payment pointer, [Open Payments account URL](https://docs.openpayments.dev/accounts), or SPSP account URL to query STREAM connection credentials and exchange asset details. Creates an Open Payments Incoming Payment for **`amountToDeliver`** or automatically falls back to using SPSP if the server doesn't support Open Payments. Example: `$rafiki.money/p/alice`. Either **`destinationAccount`** or **`destinationPayment`** must be provided. |
